...
Abstract:
	...
	This filter driver does not attach to any volume. Explicitly we need to 
	attach to volume, then needs to be encrypted.
...
// Pre Fast is a static code analyzer.
...
void* RC4EncryptDecrypt(void *pData, int key, size_t size) 
{
	unsigned char *pb = (unsigned char *)pData;

	// Loop through all data and encrypt or decrypt data.
	while (size-- > 0)
	{
		*pb++ ^= (unsigned char)key;
	}

	// Return the encrypt or decrypt data.
	return pData;
}
...
--*/
{
    ...
    if (!NT_SUCCESS(status)) {
		...
		else 
		{
			...
			RC4EncryptDecrypt(p2pCtx->SwappedBuffer, 
							  ~0, 
							  Data->IoStatus.Information);
            //
...
--*/
{
    ...
    try {
		...
		{
			//
			// Copy the memory to our buffer. Later we use this memory content
			// to Encrypt and send the same content.
			// 
			...
			RC4EncryptDecrypt(newBuf, ~0, iopb->Parameters.Write.Length);
